...
plotDT <- twDT[, .(
               ...
               ), keyby = .(obsDateTime = lubridate::floor_date(created_at, "hour")), ba_tweetType)]

  myPlot <- ggplot(plotDT, aes(x = obsDateTime) +
    geom_line(aes(y = nTweets, colour = "N tweets")) +
    geom_line(aes(y = nTweeters, colour = "N tweeters")) +
    facet_grid(ba_tweetType ~ .) +
    scale_x_date(date_breaks = "1 day", date_labels = "%a %d %b %Y %HH") +
