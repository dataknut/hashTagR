 ...
 * Anderson, B (`r lubridate::year(today())`) _Twitter hashtag analysis: `r params$searchString`_. Downloaded from: `r params$pubUrl`.
...
The idea is to extract and visualise tweets and re-tweets of _`r params$searchString`_ (see `r params$explHashTag`).
...
The analysis used `rtweet` to ask the Twitter search API to extract 'all' tweets containing the _`r params$searchString`_ hashtags in the '[recent](https://developer.twitter.com/en/docs/tweets/search/api-reference/get-search-tweets)' twitterVerse. 
...
raw_twDT <- hashTagR::loadTweets(dPath, params$searchString) # we like data.tables
...
dataCap <- paste0("Source: Data collected from Twitter's REST API via rtweet",
                  "\nAll (re)tweets and quotes containing ", params$searchString, 
                    " from ",
                      min(twDT$created_at),
                    " to ",
                    max(twDT$created_at)
...
Figure \@ref(fig:plotHashtags) plots the daily occurence of these hashtags after removing variants of `r params$searchString` and selecting only those which have more than `r threshold` mentions on any day. For clarity tweets and re-tweets are aggregated. See Section \@ref(problems) for the problems with this #hashTag counting approach.
...
for(n in 1:length(params$hashTags)){
  testDT <- testDT[!(grepl(params$hashTags[n],htClean,ignore.case = TRUE))]
}
...
```{r cumulativeHtCounts, fig.cap=`r paste0("Cumulative hashtag counts over time (only total count >", threshold, " shown)")'}
...
```{r htCountsTodate, fig.cap=`r paste0("Total hashtag counts to date (only total count > ", threshold, " shown)")'}
