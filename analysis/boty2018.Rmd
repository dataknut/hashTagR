...
The analysis used `rtweet` to ask the Twitter search API to extract 'all' tweets containing the _`r searchString`_ hashtags in the '[recent](https://developer.twitter.com/en/docs/tweets/search/api-reference/get-search-tweets)' twitterVerse. When a search is repeated the _same_ tweet can appear more than once if one of it's attributes (e.g. number of likes & re-tweets) has changed since the last search.
