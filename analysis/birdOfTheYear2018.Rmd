...
To extract and visualise tweets and re-tweets of _`r params$hashtag`_ (see `r params$explHashTag` and the [Forest & Bird voting site](https://www.birdoftheyear.org.nz/)).
...
roman <- c("a", "e", "i", "o", "u") # roman equiv

# for some reason looping over the above fails... so:
htLongDT <- htLongDT[, htOrigAnglo := gsub("ā","a", htOrig, fixed = TRUE)] 
htLongDT <- htLongDT[, htOrigAnglo := gsub("ē","e", htOrig, fixed = TRUE)] 
htLongDT <- htLongDT[, htOrigAnglo := gsub("ī","i", htOrig, fixed = TRUE)] 
htLongDT <- htLongDT[, htOrigAnglo := gsub("ō","o", htOrig, fixed = TRUE)] 
htLongDT <- htLongDT[, htOrigAnglo := gsub("ū","u", htOrig, fixed = TRUE)] 

htLongDT <- htLongDT[, htClean := tolower(htOrigAnglo)] # lower case
```

This is very quick and dirty but... Table \@ref(tab:tweetTable) shows the total count of each #hashtag. Note that I have tried to [de-macron](https://kupu.maori.nz/about/macrons-keyboard-setup) the vowels so that kakī == kaki (for example) but it is currently not working.

```{r tweetTable}
kableExtra::kable(caption = "Top 20 hashtags in tweets containing #birdoftheyear or #boty",
                  head(htLongDT[, .(count = .N), by = .(htClean, ba_tweetType)][order(-count)],20)) %>%
  kable_styling()
```

Figure \@ref(fig:plotHashtags) plots the daily occurence of these hashtags after removing variants of `#birdOfTheYear` and `#boty` and selecting only those which have more than `r threshold` mentions on any day.
...
But if the hashtags have any predictive value at all then Figure \@ref(fig:cumulativeHtCounts) should be an indicator of the direction of travel (watch for lines of apparently dis-similar hashtags where the macron fix has failed) and \@ref(fig:htCountsTodate) shows the totals to date.

\@ref(fig:cumulativeHtCounts) uses `plotly` to avoid having to render a large legend - just hover over the lines to see who is who...
...
# Problems with counting #hashtags

Loads of them. But primarily:

 * tweets mentioning birds without using a `#<birdName>` hashtag (like [this rather popular one](https://twitter.com/BillBailey/status/1046965452063293447)) will not show up;
 * we still have the kakī != kaki macron related issue.
 
> => this is a _really_ imperfect measure.  
