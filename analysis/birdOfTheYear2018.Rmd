...
Figure \@ref(fig:plotHashtags) plots the daily occurence of these hashtags after removing variants of `#birdOfTheYear` and `#boty` and selecting only those which have more than `r threshold` mentions on any day. Tweets mentioning birds without using a `#<birdName>` hashtag will not show up and we still have the kakī != kaki issue so this is a _really_ imperfect measure.  For clarity we have not separated tweets from re-tweets. #YMMV.
...
```{r cumulativeHtCounts, fig.cap="Cumulative hashtag counts"}
...
```{r htCountsTodate, fig.cap="Cumulative hashtag counts"}
